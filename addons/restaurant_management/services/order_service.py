...
class OrderService(models.AbstractModel):
    _name = 'restaurant_management.order_service'
    _description = 'Order Service'

    @api.model
    def create_order(self, vals):
        """Create a new order."""
        return self.env['restaurant_management.order'].create(vals)

    @api.model
    def read_order(self, uuid):
        """Read an order by UUID."""
        order = self.env['restaurant_management.order'].search([('uuid', '=', uuid)], limit=1)
        if order:
            return order.read()
        return False
