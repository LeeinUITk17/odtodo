...
class CategoryService(models.AbstractModel):
    _name = 'restaurant_management.category_service'
    _description = 'Category Service'

    @api.model
    def create_category(self, vals):
        return self.env['restaurant_management.category'].create(vals)

    @api.model
    def read_category(self, uuid):
        category = self.env['restaurant_management.category'].search([('uuid', '=', uuid)], limit=1)
        if category:
