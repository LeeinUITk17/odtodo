...
class BranchController(http.Controller):
    ...
    def create_branch(self, **post):
        branch = request.env['restaurant.branch.service'].create_branch(post)
    ...
    def get_branch(self, branch_id):
        branch = request.env['restaurant.branch.service'].get_branch(branch_id)
    ...
    def update_branch(self, branch_id, **post):
        branch = request.env['restaurant.branch.service'].update_branch(branch_id, post)
        return {'status': 'success', 'data': branch} if branch else {'status': 'error', 'message': 'Branch not found'}

    @http.route('/api/branches/<int:branch_id>', type='json', auth='public', methods=['DELETE'])
    def delete_branch(self, branch_id):
        success = request.env['restaurant.branch.service'].delete_branch(branch_id)
