...
class OrderItemService(models.AbstractModel):
    _name = 'order_management.orderitem_service'
    _description = 'Order Item Service'

    @api.model
    def create_orderitem(self, vals):
        return self.env['order_management.orderitem'].create(vals)

    @api.model
    def read_orderitem(self, uuid):
        orderitem = self.env['order_management.orderitem'].search([('uuid', '=', uuid)], limit=1)
        if orderitem:
