...
class MenuItemService(models.AbstractModel):
    _name = 'menu_management.menuitem_service'
    _description = 'Menu Item Service'

    @api.model
    def create_menuitem(self, vals):
        return self.env['menu_management.menuitem'].create(vals)

    @api.model
    def read_menuitem(self, uuid):
        menuitem = self.env['menu_management.menuitem'].search([('uuid', '=', uuid)], limit=1)
        if menuitem:
